        ...
        if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
            ...
            if ($image_path) {
                ...
                if (!empty($existing['image_path'])) {
                    $room->deleteImage($existing['image_path']);
                }
        ...
        <form method="POST" enctype="multipart/form-data" class="row g-3">
            <div class="col-md-6">
                ...
                <input type="text" name="name" class="form-control"
                       value="<?= htmlspecialchars($existing['name']) ?>" required>
            </div>
            <div class="col-md-6">
                <label class="form-label">Type*</label>
                <input type="text" name="type" class="form-control"
                       value="<?= htmlspecialchars($existing['type']) ?>" required>
            </div>
            <div class="col-md-4">
                <label class="form-label">Price per Night* ($)</label>
                <input type="text" name="price_per_night" class="form-control"
                       value="<?= htmlspecialchars($existing['price_per_night']) ?>" required>
            </div>
            <div class="col-md-4">
                <label class="form-label">Max Guests*</label>
                <input type="number" name="max_guests" class="form-control"
                       value="<?= htmlspecialchars($existing['max_guests']) ?>" required>
            </div>
            <div class="col-12">
            ...
            <div class="col-12">
                ...
                <?php if (!empty($existing['image_path']) && file_exists($existing['image_path'])): ?>
                    <div class="mb-2">
                        <p class="text-muted mb-2">Current Image:</p>
                        <img src="<?= htmlspecialchars($existing['image_path']) ?>" 
                ...
                <small class="text-muted">Upload a new image to replace the current one. Accepted: JPG, PNG, GIF, WEBP (Max 5MB)</small>
                
                <!-- New Image Preview -->
                <div id="imagePreview" class="mt-3" style="display: none;">
                    <p class="text-muted mb-2">New Preview:</p>
